...
import DeleteWebhookButton from "./delete-workflow-button";
...
const WorkflowForm: React.FC<IWorkFlowsProps> = ({ onSuccess, selectedWorkflow }) => {
  ...
  const onSubmit = async (values: IWorkFlows) => {
    try {
      let res;
      if (selectedWorkflow) {
        res = await api.updateWorkflow({ ...values, id: selectedWorkflow.id } as Record<string, unknown>);
      } else {
        res = await api.createWorkflow(values as unknown as Record<string, unknown>);
      }
